...
def jouer_episode(vec_env, model, render=True, delai=0.02, deterministic=True, diagnostic=False, frame_stack=1, afficher_lidar=True):
    ...
    while True:
        ...
        if render:
            ...
            if env_rendu is not None:
                env_rendu.render(afficher_lidar=afficher_lidar)
            time.sleep(delai)
...
def jouer_plusieurs_episodes(vec_env, model, n_episodes=10, render=True, delai=0.02, diagnostic=False, frame_stack=1, afficher_lidar=True):
    ...
    for i in range(n_episodes):
        resultat = jouer_episode(vec_env, model, render=render, delai=delai, diagnostic=diagnostic, frame_stack=frame_stack, afficher_lidar=afficher_lidar)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sans-lidar", action="store_true",
                         help="Coupe l'affichage des rayons LiDAR dans le rendu -- utile pour "
                              "un clip de vol propre (sans les traits colorés superposés), à "
                              "opposer à un clip fait sans ce flag pour montrer le capteur.")
    ...
    try:
        jouer_plusieurs_episodes(vec_env, model, n_episodes=args.episodes,
                                  ...
                                  frame_stack=args.frame_stack, afficher_lidar=not args.sans_lidar)
